...
public class CapabilityRegistry {
    ...
    public static void onAttachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof Player) {
            if (!event.getObject().getCapability(PlayerManaProvider.PLAYER_MANA).isPresent()) {
                event.addCapability(new ResourceLocation(MineBound.MOD_ID, "properties"), new PrimarySpellProvider());
                event.addCapability(new ResourceLocation(MineBound.MOD_ID, "primary_spell"), new SecondarySpellProvider());
            }
        }
    }
    ...
    public static void onRegisterCapabilities(RegisterCapabilitiesEvent event) {
        ...
        event.register(SelectedSpell.class);
    }
}
